...
public class SalesWindow extends javax.swing.JFrame implements ActionListener{
    ...
    private void initComponents() {
        ...
        pnlSupplierLayout.setHorizontalGroup(
            ...
            .addGroup(javax.swing.GroupLayout.Alignment.TRAILING, pnlSupplierLayout.createSequentialGroup()
                ...
                .addComponent(txtBillNumber, javax.swing.GroupLayout.PREFERRED_SIZE, 162, javax.swing.GroupLayout.PREFERRED_SIZE))
        );
    ...
    private void btnAddMoreActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAddMoreActionPerformed
        ...
        SuggestionExampleMain.populateStudentNames(this, txtCustomerName);
    ...
    private void btnSaveActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnSaveActionPerformed
        ...
        //try{
            if(btnSave.getText().equalsIgnoreCase("new")){
                ...
                System.out.println("!@#------------update **"+returnList.size());
    ...
    private void cmbCustomerNameActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmbCustomerNameActionPerformed
        // TODO add your handling code here:
        try{
            if(null == (((Staff) cmbCustomerName.getSelectedItem()).getStaffName())
                    || "new".equalsIgnoreCase(((Staff) cmbCustomerName.getSelectedItem()).getStaffName())
                    || "default".equalsIgnoreCase(((Staff) cmbCustomerName.getSelectedItem()).getStaffName())){                    
                ...
                txtDisplayName.setText(((Staff) cmbCustomerName.getSelectedItem()).getStaffName()+", \n"+((Staff) cmbCustomerName.getSelectedItem()).getAddress());
                txtItemName.requestFocus();
                txtDisplayName.setVisible(true);
                lblDisplayName.setVisible(true);
                ...
                JOptionPane.showMessageDialog(null, "Please enter the customer details");
                e.printStackTrace();
        }
    ...
    private void cmbCustomerTypeActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmbCustomerTypeActionPerformed
        ...
        try{
            ...
            if(!(btnSave.getText().equalsIgnoreCase("update"))){
                billSeqNumber = billGenDAO.generateBillNumber(billType,"2023-03-31");
                if(billType.equalsIgnoreCase("B2B")){
                    txtBillNumber.setText("FCBM-BB-"+billSeqNumber+"/23-24");
                    //txtDisplayName.requestFocus();
                }else{
                    txtBillNumber.setText("FCBM-"+billSeqNumber+"/23-24");
                }
    ...
    private void chkSameAsAboveKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_chkSameAsAboveKeyReleased
        // TODO add your handling code here:
        if(evt.getKeyCode()== KeyEvent.VK_ENTER){
           if(null!=txtShippingAddress.getText() && !("".equals(txtShippingAddress.getText())))
                txtVehicleDetails.requestFocus();
           else
    ...
    public void saveSalesForm(){
        ...
        if(null ==txtCustomerName.getText() || "".equalsIgnoreCase(txtCustomerName.getText())){
                Staff cust = null;
                StaffDAO custDAO = null;
                custDAO = new StaffDAO();
                cust = custDAO.retrieveByName("New");
                cust.setStaffName("");
                cust.setPhoneNumber("1111100000");
                cust.setAddress(txtDisplayName.getText());
                gCustomer = cust;
        }
        else if (null !=txtCustomerName.getText()){
                Staff cust = null;
                StaffDAO custDAO = null;
                custDAO = new StaffDAO();
                cust = custDAO.retrieveByName(txtCustomerName.getText());
                if(cust==null){
                    StaffDAO customerDAO = new StaffDAO();
                    Staff customer = new Staff();
                    customer.setStaffName(txtCustomerName.getText());
                    customer.setPhoneNumber("1111100000");
                    customer.setAddress(txtDisplayName.getText());
                    customerDAO.insertIntoDB(customer);
                    custDAO = new StaffDAO();
                    gCustomer = custDAO.retrieveByName(txtCustomerName.getText());
                }
        }
        System.out.println("----CUST ID--"+gCustomer.getStaffId());
        Staff customer = gCustomer;
        sales.setCustomerId(customer.getStaffId());
    ...
    public void updateSalesAfterReturn(List<SalesReturn> returnList){
        System.out.println("!@#------INSIDE updateSalesAfterReturn-----");
        //SalesDAO billGenDAO =  new SalesDAO();
        //!@-- 03/15/2020
        ...
        sales.setSalesDate(dtpSalesDate.getDate());
        /*Customer customer = (Customer)cmbCustomerName.getSelectedItem();*/
        //Customer gCustomer = (Customer)cmbCustomerName.getSelectedItem();
        sales.setCustomerId(gCustomer.getStaffId());
        System.out.println("----"+sales.getBillNumber());
        sales.setBillAmount(Double.parseDouble(txtTotalAmount.getText()));
        sales.setDiscount(Double.parseDouble(txtDiscountAmount.getText()));
        sales.setPayableAmount(Double.parseDouble(txtAmountPayable.getText()));
        ...
        System.out.println("Before Sale Items loop");
    ...
    public void renderDataIntoDropDown(List<PricingTier> pricingTierList){
        ...
        public void renderCustomerDataIntoDropDown(List<Staff> customerList){
        Staff customer = null;
        //cmbCustomerName.removeAllItems();
        ...
        if(customerList!= null){
            ...
            customer = (Staff)cmbCustomerName.getSelectedItem();
            //cmbCustomerType.gets
            System.out.println("----------"+cmbCustomerName.getSelectedItem()+"::"+customer.getStaffName());
        }
    }
    ...
    public final void fetchSalesDataIntoTable(Sales sales){
        ...
        //!@#-- 20/11/2022 To retrieve and display customer details in text box
        StaffDAO customerDAO = new StaffDAO();
        gCustomer=null;
        gCustomer=customerDAO.retrieveById(sales.getCustomerId());        
        txtCustomerName.setText(gCustomer.getStaffName());
        //gCustomer=null;
        txtDisplayName.setText(sales.getCustomerName());
        SuggestionExampleMain.populateStudentNames(this, txtCustomerName);
        //--Populating pricing tier
        PricingTierDAO pricingTierDAO = new PricingTierDAO();
        renderDataIntoDropDown(pricingTierDAO.retrieveAll());
        //--------------
    ...
    public void populateItemDetails(){
     ...
     public void populateCustomerDetails(){
            System.out.println("---inside Customer name change");
            try{
                StaffDAO customerDAO = null;
                gCustomer = null;
                if(!txtCustomerName.getText().trim().equalsIgnoreCase(""))
                customerDAO = new StaffDAO();
                gCustomer = customerDAO.retrieveByName(txtCustomerName.getText().trim());
                txtDisplayName.setText(gCustomer.getStaffName()+", \n"+gCustomer.getAddress());                    
            }            
            catch(Exception e){
                System.out.println("Error Retrieving the Customer -"+e.getMessage());
            }            
    ...
    public void customerSelectionEvent(){
    try{
            if(null == txtCustomerName.getText()
                    || "new".equalsIgnoreCase(txtCustomerName.getText())
                    || "default".equalsIgnoreCase(txtCustomerName.getText())){                    
                txtDisplayName.setText("");
                txtShippingAddress.setText("");
                txtDisplayName.setVisible(true);
                lblDisplayName.setVisible(true);
                ...
                txtDisplayName.setText(txtCustomerName.getText()+", \n"+ gCustomer.getAddress());
                txtItemName.requestFocus();
                txtDisplayName.setVisible(true);
                lblDisplayName.setVisible(true);
                txtVehicleDetails.requestFocus();
    ...
    public void actionPerformed(ActionEvent event) {
        try{
            ...
            if (menu == mitDelete) {
                ...
                SuggestionExampleMain.populateStudentNames(this, txtCustomerName);
                PricingTierDAO pricingTierDAO = new PricingTierDAO();
                //!@#- 18/11/2022 -- renderDataIntoDropDown(pricingTierDAO.retrieveAll());
                txtItemName.requestFocus();
                //########
                salesReturn.setBillNumber(txtBillNumber.getText().trim());
                salesReturn.setItemName(tblSalesDetails.getModel().getValueAt(tblSalesDetails.getSelectedRow(),1).toString());
